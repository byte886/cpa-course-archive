...
5. **识别用户指定的角色**（开发工程师/项目架构师/测试工程师/产品经理），根据角色确定职责和关注点：
...
**用户启动新对话的最简方式**：
- 用户只需要把项目文件夹加入新对话
- 然后说角色名称即可（如"开发工程师"、"项目架构师"、"测试工程师"、"产品经理"）
...
| **测试工程师** | **不负责管理**，只按项目架构师设计的目录结构存放 | 测试文档、测试用例、bug报告、测试报告 | 功能测试、回归测试、性能测试、验证修复、知识库质量检查 |
| **产品经理** | **不负责管理**，只按项目架构师设计的目录结构存放 | 需求文档、产品规划、用户反馈、产品路线图、业务知识整理 | 需求分析、产品规划、用户研究、优先级排序、PRD编写 |

**核心原则**：
- **管理统一由项目架构师负责**：目录结构设计、文件命名规范、文档体系规划、流程管理、文档治理等所有管理工作，统一由项目架构师负责
- **其他角色只负责写**：开发工程师、测试工程师、产品经理只负责写自己相关的内容，不负责管理目录结构或文档体系
- **变更驱动更新**：
  - 目录结构/流程变化 → 项目架构师负责更新相关文档和目录结构
  - 功能变化 → 开发工程师负责写技术文档，项目架构师负责检查是否需要更新目录结构
  - 测试发现bug → 测试工程师负责写bug报告，项目架构师负责检查是否需要更新流程文档
