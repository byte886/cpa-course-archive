...
4. 读取`project-management/active/任务交接文档.md`（如果有）
5. **根据任务交接文档中的"下一步"部分，自动确定当前任务并开始执行，不需要用户额外提示**
6. 不要依赖对话记忆，所有重要信息必须从文档中获取

**用户启动新对话的最简方式**：
- 用户只需要把项目文件夹加入新对话，然后说"继续"或"接手项目"
- 不需要用户手动复制启动话术，AI应自动读取文档并确定当前任务
- 如果用户有特殊要求（如"先做某件事"），则按用户要求执行
