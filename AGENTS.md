...
5. **根据用户的具体要求确定职责**：
   - 如果用户说"继续"或"接手"，则按交接文档中的"建议的下一步任务"执行
   - 如果用户有其他要求（检查、优化、修复、文档整理等），则按用户要求执行
   - 如果用户没有明确要求，先总结项目状态和建议的下一步，和用户确认后再执行
6. 不要依赖对话记忆，所有重要信息必须从文档中获取

**用户启动新对话的最简方式**：
- 用户只需要把项目文件夹加入新对话
- 然后说具体要做什么（如"继续"、"检查状态"、"优化文档"等）
- 不需要用户手动复制启动话术，AI应自动读取文档并理解项目状态
- 如果用户没有明确要求，AI应先总结项目状态和建议的下一步，和用户确认
