...
5. **识别用户指定的角色**（开发人员/项目管理/测试工程师/产品经理），根据角色确定职责和关注点：
   - **开发人员**：关注技术实现、代码结构、待开发功能、技术问题
   - **项目管理**：关注项目进度、任务状态、风险问题、文档完整性、下一步计划
...
**用户启动新对话的最简方式**：
- 用户只需要把项目文件夹加入新对话
- 然后说角色名称即可（如"开发人员"、"项目管理"、"测试工程师"、"产品经理"）
...
| 角色 | 管理责任 | 编写责任 | 其他责任 |
|------|---------|---------|---------|
| **项目管理** | **统一负责所有管理工作**：目录结构设计、文档体系规划、流程管理、文档治理、目录结构一致性维护、文档关联性检查、ADR决策管理 | 项目文档、流程文档、交接文档、规范文档、ADR决策 | 项目进度管理、风险管理、质量保障、跨角色协调 |
| **开发人员** | **不负责管理**，只按项目管理设计的目录结构实现 | 技术文档、脚本说明、API文档、代码注释 | 功能开发、bug修复、性能优化、自动化流程实现 |
| **测试工程师** | **不负责管理**，只按项目管理设计的目录结构存放 | 测试文档、测试用例、bug报告、测试报告 | 功能测试、回归测试、性能测试、验证修复 |
| **产品经理** | **不负责管理**，只按项目管理设计的目录结构存放 | 需求文档、产品规划、用户反馈、产品路线图 | 需求分析、产品规划、用户研究、优先级排序 |

**核心原则**：
- **管理统一由项目管理负责**：目录结构设计、文档体系规划、流程管理、文档治理等所有管理工作，统一由项目管理负责
- **其他角色只负责写**：开发人员、测试工程师、产品经理只负责写自己相关的内容，不负责管理目录结构或文档体系
- **变更驱动更新**：
  - 目录结构/流程变化 → 项目管理负责更新相关文档和目录结构
  - 功能变化 → 开发人员负责写技术文档，项目管理负责检查是否需要更新目录结构
  - 测试发现bug → 测试工程师负责写bug报告，项目管理负责检查是否需要更新流程文档
  - 需求变化 → 产品经理负责写需求文档，项目管理负责检查是否需要更新项目规划
- **文档问题发现即修复**：任何角色发现文档问题都要及时修复，但管理类变更（目录结构、文档体系）需要项目管理统一规划
